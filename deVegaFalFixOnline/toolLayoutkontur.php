...
$selectBilderextrasSql = "SELECT uid, tx_kbks_lk_file FROM tx_kbks_layoutkontur WHERE deleted=0";
$selectBilderextras = $dbConn->query($selectBilderextrasSql); 

while($dataRow = $selectBilderextras->fetch_assoc()) {	//print_r($dataRow); die();
	$uid		= $dataRow['uid'];
	$imageTitle	= $dataRow['tx_kbks_lk_file'];	
	if($imageTitle){
		$fileName		= $identifier = $extension = '';
		$fileName		= trim($imageTitle);
		$identifier		= $allFilePath.$fileName;
		$extension		= pathinfo($fileName, PATHINFO_EXTENSION); 
		_insert_each_sys_file($identifier, $fileName, $extension, $uid, 'tx_kbks_lk_file');
	}
}
	...
	function _insert_product_mm_sys_file_reference($uid_local, $uid_foreign, $fieldname, $title ) {
		...
		$insertSysfileRefSql .= '('.$uid_local.',"'.$uid_foreign.'","tx_kbks_layoutkontur","'.$fieldname.'","sys_file",'.$pid.',"'.$title.'",8887,8887,'.$timeNow.','.$timeNow.')';
