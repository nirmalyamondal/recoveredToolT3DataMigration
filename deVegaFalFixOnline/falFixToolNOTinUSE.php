...
$selectBilderextrasSql = "SELECT uid, tx_kbks_px_image FROM tx_kbks_bilderextras WHERE deleted=0";
$selectBilderextras = $dbConn->query($selectBilderextrasSql); 

while($dataRow = $selectBilderextras->fetch_assoc()) {	//print_r($dataRow); die();
	$uid		= $dataRow['uid'];
	$imageTitle	= $dataRow['tx_kbks_px_image'];	
	if($imageTitle){
		$fileName		= $identifier = $extension = '';
		$fileName		= trim($imageTitle);
		$identifier		= $allFilePath.$fileName;
		$extension		= pathinfo($fileName, PATHINFO_EXTENSION); 
		_insert_each_sys_file($identifier, $fileName, $extension, $uid, 'tx_kbks_px_image');
	}
}
	...
	function _insert_product_mm_sys_file_reference($uid_local, $uid_foreign, $fieldname, $title ) {
		...
		$insertSysfileRefSql .= '('.$uid_local.',"'.$uid_foreign.'","tx_kbks_bilderextras","'.$fieldname.'","sys_file",'.$pid.',"'.$title.'",8888,8888,'.$timeNow.','.$timeNow.')';
